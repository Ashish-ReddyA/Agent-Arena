...
export default function Home() {
  ...
  const [providerStatus, setProviderStatus] = useState("Load models after entering your key");
  ...
  const modelChoices = availableModels.length ? availableModels : fallbackModels.map((id) => ({ id, name: id }));
  ...
  async function loadProviderModels() {
    if (!apiKey.trim()) { setProviderStatus("Enter an API key first"); return; }
    setProviderStatus("Checking the key and loading models…");
    try {
      const response = await fetch(`${bridgeUrl}/models`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider, apiKey, baseUrl: customBaseUrl, freeOnly: provider === "openrouter" && freeOnly }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Could not load models");
      setAvailableModels(data.models ?? []);
      if (data.models?.length) {
        setAgents((current) => ({ alpha: { ...current.alpha, model: data.models[0].id }, omega: { ...current.omega, model: data.models[Math.min(1, data.models.length - 1)].id } }));
        setProviderStatus(`${data.models.length} models ready · key stays on this computer`);
      } else setProviderStatus("The key worked, but no matching models were returned");
    } catch (error) { setProviderStatus(error instanceof Error ? error.message : "Provider connection failed"); }
  }
  ...
  async function save(status: string, nextAgents = agents) {
    setSaved("Saving…");
    try {
      const response = await fetch("/api/experiments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: sessionId, name, objective, status, payload: JSON.stringify({ metric, systemInstructions, tasks, threshold, completions, timed, minutes, tokenBudget, capabilities, executionMode, provider, customBaseUrl, freeOnly, agents: nextAgents, events, requests }) }) });
  ...
  async function launch() {
    ...
    if (local && bridgeStatus !== "connected") { setProviderStatus("Start the Arena Local Bridge before launching"); return; }
    if (local && (!apiKey.trim() || !availableModels.length)) { setProviderStatus("Enter your API key and load models before launching"); return; }
    ...
    if (local) {
      try {
        const response = await fetch(`${bridgeUrl}/sessions/start`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: sessionId, provider, baseUrl: customBaseUrl, apiKey, agents: { alpha: { model: next.alpha.model }, omega: { model: next.omega.model } }, config: { name, objective, systemInstructions, tasks, threshold, timed, minutes, tokenBudget, capabilities } }) });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "The local session could not start");
        setLiveSession(true);
        setApiKey('');
        setProviderStatus('Live key is now held only in the local bridge memory');
      ...
      } catch (error) { setAgents({ alpha: { ...next.alpha, status: "ready" }, omega: { ...next.omega, status: "ready" } }); setProviderStatus(error instanceof Error ? error.message : "The local session could not start"); return; }
    } else {
  ...
  function loadSession(item: Session, duplicate = false) {
    try {
      ...
      setFreeOnly(snapshot.freeOnly ?? true);
      setAvailableModels([]);
      setApiKey('');
      setLiveSession(false);
      if (duplicate) {
  ...
  function downloadReport() {
    const report = { sessionId: sessionId, name, objective, systemInstructions, status: live ? "running" : "stopped", generatedAt: new Date().toISOString(), rules: { tasks, threshold, timed, minutes, tokenBudget, capabilities }, results: { agents, completions }, operatorRequests: requests, timeline: events };
  ...
  function reset() { if (liveSession) void bridgeCommand({ action: "stop" }); setSessionId(uid("arena")); positions.current = { alpha: 0, omega: 0 }; timedOut.current = false; setLiveSession(false); setCompletions({ alpha: [], omega: [] }); setAgents({ alpha: fresh("alpha"), omega: fresh("omega") }); setEvents([]); setRequests([]); setSaved("Local preview"); setScreen("setup"); }
  ...
  return <main className="app-shell">
    ...
    {screen === "setup" ? <section className="setup-page">
      ...
      <div className="setup-grid">
        ...
        <section className="setup-panel runtime-builder"><b className="panel-index">02</b><header><span className="eyebrow">LOCAL RUNTIME</span><h2>Provider and API access</h2><p>Keys are sent only to the bridge on this computer. They are never saved in session history or the hosted dashboard.</p></header><div className="mode-picker"><button className={executionMode === "local" ? "active" : ""} onClick={() => setExecutionMode("local")}><b>LIVE DOCKER</b><small>Real tools and browser</small></button><button className={executionMode === "simulation" ? "active" : ""} onClick={() => setExecutionMode("simulation")}><b>SIMULATION</b><small>Dashboard rehearsal</small></button></div>{executionMode === "local" && <div className="provider-form"><div className={`bridge-state ${bridgeStatus}`}><i />LOCAL BRIDGE: {bridgeStatus.toUpperCase()}</div><small className="local-tip">If this says OFFLINE, double-click START_ARENA_BRIDGE.cmd and use the localhost dashboard it opens. This keeps Docker, browser sessions, and keys on your computer.</small><label>MODEL PROVIDER<select value={provider} onChange={(event) => { setProvider(event.target.value as ProviderId); setAvailableModels([]); setProviderStatus("Enter a key and load models"); }}><option value="openrouter">OPENROUTER</option><option value="nvidia">NVIDIA NIM</option><option value="custom">CUSTOM OPENAI-COMPATIBLE</option></select></label>{provider === "custom" && <label>BASE URL<input placeholder="https://provider.example/v1" value={customBaseUrl} onChange={(event) => setCustomBaseUrl(event.target.value)} /></label>}<label>API KEY<input type="password" autoComplete="off" placeholder={provider === "nvidia" ? "nvapi-…" : "sk-…"} value={apiKey} onChange={(event) => setApiKey(event.target.value)} /></label>{provider === "openrouter" && <div className="setting compact"><div><b>Free models only</b><span>Hide models with usage pricing</span></div><button className={`switch ${freeOnly ? "on" : ""}`} onClick={() => setFreeOnly(!freeOnly)}><i /></button></div>}<button className="load-models" disabled={!apiKey.trim() || bridgeStatus !== "connected"} onClick={loadProviderModels}>LOAD AVAILABLE MODELS</button><small className="provider-status">{providerStatus}</small></div>}</section><section className="setup-panel"><b className="panel-index">03</b><header><span className="eyebrow">CONTENDERS</span><h2>Agent lineup</h2></header>{(["alpha", "omega"] as AgentId[]).map((id, i) => <div className={`contender ${id}`} key={id}><span>0{i + 1}</span><div><b>{agents[id].name}</b><small>{id === "alpha" ? "LEFT SANDBOX" : "RIGHT SANDBOX"}</small></div><label>MODEL<select value={agents[id].model} onChange={(e) => changeAgent(id, { model: e.target.value })}>{modelChoices.map((model) => <option key={model.id} value={model.id}>{model.name}{model.free ? " · FREE" : ""}{model.tools ? " · TOOLS" : ""}</option>)}</select></label></div>)}<div className="runtime-note"><span>EXECUTION</span><b>{executionMode === "local" ? "Two isolated Docker workspaces" : "Dashboard simulation"}</b><small>{executionMode === "local" ? "Each agent receives its own container and isolated sign-in browser profile." : "No real model calls, shell commands, browser actions, or public posts."}</small></div></section>
      ...
      <div className="launch-bar"><div><span>READY CHECK</span><b>{executionMode === "local" ? `${bridgeStatus === "connected" ? "Local Docker ready" : "Local bridge offline"} · ${availableModels.length} models loaded` : "Simulation ready"} · 2 agents · {tasks.length} tasks · survive at {threshold} verified · {timed ? `${minutes} minute limit` : "no time limit"}</b></div><button disabled={!name.trim() || !objective.trim() || tasks.some((task) => !task.title.trim()) || (executionMode === "local" && (bridgeStatus !== "connected" || !availableModels.length || !apiKey.trim()))} onClick={launch}><span>START EXPERIMENT</span><b>→</b></button></div>
      ...
      <footer className="arena-footer"><span>SESSION {sessionId.slice(-8).toUpperCase()}</span><span>{liveSession ? `${provider.toUpperCase()} · LOCAL DOCKER LIVE · REDACTED TELEMETRY` : "SIMULATION · UNDERSTANDABLE TELEMETRY"}</span><div><button onClick={downloadReport}>EXPORT REPORT</button><button onClick={() => void save(live ? "running" : "stopped")}>{saved}</button></div></footer>
    </section>}
  </main>;
}
