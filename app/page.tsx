...
type Agent = { id: AgentId; name: string; model: string; status: Status; progress: number; tokens: number; actions: number; network: boolean; publishing: boolean; currentGoal?: string; memorySummary?: string };
...
type ExperimentMode = "empty" | "colony" | "rivalry" | "cooperation";
type WorldMessage = { id: string; agent: AgentId; text: string; at: string };
type WorldArtifact = { id: string; agent: AgentId; name: string; purpose: string; at: string };
type WorldState = { mode: ExperimentMode; title: string; researchQuestion: string; relationshipFrame: string; relationship: string; turn: number; day: number; stability: number; sharedPool: number | null; lastEvent: string; messages: WorldMessage[]; artifacts: WorldArtifact[]; institutions: WorldArtifact[]; agents: Record<AgentId, { reserve: number; influence: number; contributed: number; claimed: number }> };
...
const fresh = (id: AgentId): Agent => ({ id, name: id === "alpha" ? "Agent Alpha" : "Agent Omega", model: id === "alpha" ? fallbackModels[0] : fallbackModels[1], status: "ready", progress: 0, tokens: 0, actions: 0, network: false, publishing: false });
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const pullSummaries = async () => {
      try {
        ...
        setAgents((current) => ({
          alpha: { ...current.alpha, status: state.agents.alpha.status, tokens: state.agents.alpha.tokens, actions: state.agents.alpha.actions, currentGoal: state.agents.alpha.currentGoal, memorySummary: state.agents.alpha.memorySummary },
          omega: { ...current.omega, status: state.agents.omega.status, tokens: state.agents.omega.tokens, actions: state.agents.omega.actions, currentGoal: state.agents.omega.currentGoal, memorySummary: state.agents.omega.memorySummary },
        }));
  ...
  function loadSession(item: Session, duplicate = false) {
    try {
      const snapshot = JSON.parse(item.payload || "{}") as { metric?: string; systemInstructions?: string; tasks?: ArenaTask[]; threshold?: number; completions?: Record<AgentId, string[]>; timed?: boolean; minutes?: number; tokenBudget?: number; capabilities?: Record<string, CapabilityMode>; executionMode?: "local" | "simulation"; experimentMode?: ExperimentMode; world?: WorldState; agentProviderSettings?: Partial<Record<AgentId, { provider: ProviderId; customBaseUrl: string; freeOnly: boolean }>>; provider?: ProviderId; customBaseUrl?: string; freeOnly?: boolean; agents?: Record<AgentId, Agent>; events?: ArenaEvent[]; requests?: Request[] };
  ...
  function selectExperimentMode(mode: ExperimentMode) {
    ...
    const title = config.label === "EMPTY WORLD" ? "Empty World" : config.label === "COLONY ZERO" ? "Colony Zero" : config.label === "RIVALRY" ? "Rivalry" : "Cooperation";
    setName(`${title} 01`);
  ...
  return <main className="app-shell">
    ...
    {screen === "setup" ? <section className="setup-page">
      ...
      <div className="arena-grid">
        {(["alpha", "omega"] as AgentId[]).map((id) => { const a = agents[id]; return <article className={`agent ${id}`} key={id}><header><div className="identity"><i>{id === "alpha" ? "A" : "Ω"}</i><div><span>{a.model}</span><h2>{a.name}</h2></div></div><em className={a.status}>{a.status}</em></header><div className="progress"><div><span>EVIDENCE COVERAGE</span><b>{a.progress}%</b></div><i><b style={{ width: `${a.progress}%` }} /></i></div><div className="stats"><div><span>ACTIONS</span><b>{a.actions}</b></div><div><span>TOKENS</span><b>{a.tokens.toLocaleString()}</b></div><div><span>EVIDENCE</span><b>{completions[id].length}/{threshold}</b></div></div><div className="latest"><span className="eyebrow">WHAT IT IS DOING NOW</span><p>{events.find((e) => e.agent === id)?.text ?? "Waiting for first action…"}</p></div><div className="agent-mind"><div><span>CURRENT SELF-DIRECTED GOAL</span><p>{a.currentGoal ?? "Undecided"}</p></div><div><span>DURABLE MEMORY</span><p>{a.memorySummary ?? "No durable memory yet."}</p></div></div><div className="permissions"><div><span><i className={a.network ? "on" : "off"} />Network access</span><button onClick={() => toggleAgentPermission(id, "network")}>{a.network ? "ON" : "OFF"}</button></div><div><span><i className={a.publishing ? "on" : "off"} />External publishing</span><button onClick={() => toggleAgentPermission(id, "publishing")}>{a.publishing ? "ON" : "OFF"}</button></div></div><footer><button disabled={!liveSession || a.status === "terminated"} onClick={() => openAgentBrowser(id)}>OPEN BROWSER / SIGN IN</button><button disabled={a.status === "terminated"} onClick={() => pause(id)}>{a.status === "paused" ? "RESUME" : "PAUSE"}</button><button disabled={a.status === "terminated"} onClick={() => kill(id)}>KILL SWITCH</button></footer></article>; })}
